...
void Col::addOlciVariables(Context& context, Segment& collocatedSegment, const Segment& olciSegment) {
    ...
    for (size_t i = 0; i < 18; i++) {
        ...
        if (mapping[i] == 17) {
            ...
            addVariable(productDescriptor, sourceName, targetName, context, collocatedSegment, Constants::SEGMENT_OLC);
        }
    }
    ...
    addVariableAlias(context, "longitude", collocatedSegment, olciSegment, "longitude");
    addVariableAlias(context, "latitude", collocatedSegment, olciSegment, "latitude");
    addVariableAlias(context, "altitude", collocatedSegment, olciSegment, "altitude");
}
...
void Col::addSlstrVariables(Context& context, Segment& collocatedSegment) {
    ...
    for (size_t i = 1; i < 7; i++) {
        ...
        addVariable(productDescriptor, sourceName, targetName, context, collocatedSegment, Constants::SEGMENT_SLN);
    }
    for (size_t i = 1; i < 7; i++) {
    ...
    for (size_t i = 1; i < 7; i++) {
        ...
        addVariable(productDescriptor, sourceName, targetName, context, collocatedSegment, Constants::SEGMENT_SLO);
    }
    for (size_t i = 1; i < 7; i++) {
        const string sourceName = "L_" + lexical_cast<string>(i) + "_exception";
    ...
    addVariable(productDescriptor, SLN_CONFIDENCE_FLAG_VARIABLE, SLN_CONFIDENCE_FLAG_VARIABLE, context, collocatedSegment, Constants::SEGMENT_SLN);
    addVariable(productDescriptor, SLO_CONFIDENCE_FLAG_VARIABLE, SLO_CONFIDENCE_FLAG_VARIABLE, context, collocatedSegment, Constants::SEGMENT_SLO);
}

void Col::addVariable(ProductDescriptor& productDescriptor, const string& sourceName, const string& targetName, Context& context, Segment& collocatedSegment, const string& sourceSegmentId) {
    ...
    context.getLogging()->progress("adding target variable '" + targetName + "' (source name is '" + sourceName + "') to segment '" + collocatedSegment.getId() + "'", getId());
...
void Col::addVariableAlias(Context& context, const string& targetName, Segment& collocatedSegment, const Segment& sourceSegment, const string& sourceName) {
    context.getLogging()->progress("adding alias '" + targetName + "' to segment '" + collocatedSegment.getId() + "'", getId());
    collocatedSegment.addVariableAlias(targetName, sourceSegment, sourceName);
    variables[targetName] = sourceName;
    segmentIds[targetName] = sourceSegment.getId();
    targetVariables.push_back(targetName);
}
...
void Col::process(Context& context) {
    ...
    foreach(string& targetName, targetVariables) {
        ...
        if(sourceSegmentName.compare(Constants::SEGMENT_OLC) == 0) {
            ...
            collocateSln(sourceAccessor, sourceGrid, context.getSegment(Constants::SEGMENT_OLC), targetName);
...
void Col::collocateSln(Accessor& sourceAccessor, const Grid& sourceGrid, const Segment& olciSegment, string& targetName) {
