...
public:
	E1(const Pixel& p, int16_t amin, Context& context) :
			p(p), amin(amin), context(context), pn(10), p0(10), pe(10), u(valarray<double>(10), 10) {
	}

	double getValue(double tau550) {
		...
		LineMinimizer lineMinimizer(em, pn, u);
		MultiMin::powell(em, lineMinimizer, pn, p0, pe, u, 1.0e-4, 100);

		return em.getValue(pn);
	}

	const valarray<double>& getParameters() {
		return pn;
	}
