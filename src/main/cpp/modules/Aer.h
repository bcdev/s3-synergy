...
private:
	...
	map<size_t, shared_ptr<Pixel> >& getPixels(Context& context, long firstL, long lastL, map<size_t, shared_ptr<Pixel> >& pixels) const;
	void putPixels(map<size_t, shared_ptr<Pixel> >& pixels) const;
